...
class store(commands.Cog):
  ...
  async def iteminfo(self, ctx, id: typing.Optional[int], *,
    ...
    item = None
    if id:
      item = await client.db.fetchrow(
          'SELECT * FROM store WHERE id = $1 AND guild_id = $2', id,
          ctx.guild.id)
    elif name:
      item = await client.db.fetchrow(
          "SELECT * FROM store WHERE LOWER(name) LIKE '%' || $1 || '%' AND guild_id = $2",
          name, ctx.guild.id)

    if item is None:
      ecoembed.description = "<:pixel_error:1187995377891278919> Cant Find Item In Store."
    ...
    ecoembed.add_field(
        name='Price',
        value=
        f"{pvc_coin(ctx.guild.id)[0] if item['currency'] == 2 else coin(ctx.guild.id)} {item['price']}"
    )

    reward = (" ,".join([
        (ctx.guild.get_role(role_id)).mention
        for role_id in i['roles'] if (ctx.guild.get_role(role_id))
    ]) if i['roles'] else "") + "\n" + (
        f"- {pvc_coin(ctx.guild.id)[0]} {i['pvc']}" if i['pvc'] != 0 else ""
    ) + "\n" + (f"- {coin(ctx.guild.id)} {i['cash']} cash" if i['cash'] != 0
                else "") + "\n" + (f"- {coin(ctx.guild.id)} {i['bank']} bank"
                                   if i['bank'] != 0 else "")

    ecoembed.add_field(name='Reward(s)', value=reward)

    if i['rroles']:
      ecoembed.add_field(name='Requirement(s)',
                         value=" ,".join([(ctx.guild.get_role(role_id)).mention
                                          for role_id in i['rroles']
                                          if (ctx.guild.get_role(role_id))])
                         if i['rroles'] else "None")
  ...
  async def store(self, ctx):
    i = 0
    doc = await client.db.fetch(
        "SELECT * FROM store WHERE guild_id = $1 ORDER BY currency DESC , price DESC ",
        ctx.guild.id)
    ...
    ecoembed.set_author(name=f"{ctx.guild.name} STORE",
    ...
    for x in doc:
      ...
      ecoembed.add_field(
          name=
          f'{coin(ctx.guild.id) if x["currency"] == 1 else pvc_coin(ctx.guild.id)[0]} {x["price"]} - {x["name"]} , `id-{x["id"]}`',
          value=f'{x["info"]}{limit}\n<:inv:1148131171885142086>',
          inline=False)
    await ctx.send(embed=ecoembed)
  ...
  @commands.check(check_channel)
  @cooldown(1, 5, BucketType.user)
  async def buyitem(self, ctx, id: typing.Optional[int], *,
                    name: typing.Optional[str]):
    ecoembed = discord.Embed(color=0x08FC08)
    ecoembed.set_author(name=ctx.author,
                        icon_url=ctx.author.display_avatar.url)
    ...
    # Checking if the user has already bought the item or not
    if item['buyers'] and ctx.author.id in item['buyers']:
      if item['roles']:
        try:
          await ctx.author.add_roles(*[
              ctx.guild.get_role(role) for role in item['roles']
              if ctx.guild.get_role(role)
              and ctx.guild.get_role(role) not in ctx.author.roles
          ])
        except:
          await ctx.send(
              "<:pixel_error:1187995377891278919> You have already bought this item but there was an error while giving you the roles for this item. Please inform admins to put the bot role above reward role and give manage role perms to the bot!"
          )
          return
      ecoembed.description = "<:pixel_error:1187995377891278919> You have already bought this item and your roles have already been added if there were any of them pending to be added."
      await ctx.send(embed=ecoembed)
      return

    bal = await self.client.db.fetchrow(
        'SELECT * FROM users WHERE id = $1 AND guild_id = $2 ', ctx.author.id,
        ctx.guild.id)
    if bal is None:
    ...
    if item['rroles']:
      for rrole in item['rroles']:
        if ctx.guild.get_role(rrole) and ctx.guild.get_role(
            rrole) not in ctx.author.roles:
          await ctx.send(embed=bembed(
              f'You Dont Have the Req. Role i.e {ctx.guild.get_role(rrole).mention}'
          ))
          return

    if item['limit'] != None and item['limit'] <= 0:
      ecoembed.description = "⚠️ This item is out of Stock."
      await ctx.send(embed=ecoembed)
      return
      ...
      if t == 'cash':
        await client.db.execute(
            "UPDATE users SET cash = cash - $1 WHERE id = $2 AND guild_id = $3",
            item["price"], ctx.author.id, ctx.guild.id)
      elif t == 'pvc':
        await client.db.execute(
            "UPDATE users SET pvc = pvc - $1 WHERE id = $2 AND guild_id = $3",
            item["price"], ctx.author.id, ctx.guild.id)

      await client.db.execute(
          "UPDATE store SET buyers = array_append(buyers, $1) WHERE id = $2 AND guild_id = $3",
          ctx.author.id, itemid, ctx.guild.id)
      ...
      if item['roles']:
        try:
          await ctx.author.add_roles(*[
              ctx.guild.get_role(r_role) for r_role in item['roles']
              if ctx.guild.get_role(r_role)
          ])
        except:
          await ctx.send(
              "<:pixel_error:1187995377891278919> There was an error in giving you the rewarded roles. Please inform admins to put the bot role above reward role and give manage role perms to the bot and buy the item again you won't be charged this time."
          )

      await client.db.execute(
          "UPDATE users SET cash = cash + $1 , bank = bank + $2 , pvc = pvc + $3 WHERE id = $4 AND guild_id = $5",
          item["cash"], item['bank'], item['pvc'], ctx.author.id, ctx.guild.id)
      if item['limit'] is not None:
        await client.db.execute(
            'UPDATE store SET "limit" = "limit" - 1 WHERE id = $1 AND guild_id = $2',
            itemid, ctx.guild.id)
