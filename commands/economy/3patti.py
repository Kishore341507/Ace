...
async def get_bal(user_id , guild_id) :
    bal = await client.db.fetchrow('SELECT cash , bank FROM users WHERE id = $1 AND guild_id = $2 ', user_id , guild_id)
    return bal or {"cash" : 0 , "bank" : 0}
...
class myview(discord.ui.View):
    ...
    async def winner_update(self , winner):
        ...
        # await coll.update_one({"id" :self.user[0]['user'].id } , {"$inc" : { "bank" : + self.amounts[self.user[0]['user']][2] }})
        # await coll.update_one({"id" :self.user[1]['user'].id } , {"$inc" : { "bank" : + self.amounts[self.user[1]['user']][2] }})
        ...
        if self.user[0]['user'] == winner :
            ...
            await client.db.execute("UPDATE users SET bank = bank + $1 WHERE id = $2 AND guild_id = $3", (self.amounts[self.user[0]['user']][1] + self.amounts[self.user[1]['user']][1]) , self.user[0]['user'].id , self.user[0]['channel'].guild.id)
            
        elif  self.user[1]['user'] == winner :
            text1 = " "
            text2 = "👑"   
            text13 = self.pattern( self.user[1]['user'])
            await client.db.execute("UPDATE users SET bank = bank + $1 WHERE id = $2 AND guild_id = $3", (self.amounts[self.user[0]['user']][1] + self.amounts[self.user[1]['user']][1]) , self.user[1]['user'].id , self.user[1]['channel'].guild.id)
    ...
    async def bet_2(self , interaction , button):
        ...
        self.amounts[self.current][2] = (await get_bal(self.current.id , interaction.guild.id ))['bank']    
        ...
        if not self.blind_info[self.current]  :
            ...
            await client.db.execute("UPDATE users SET bank = bank - $1 WHERE id = $2 AND guild_id = $3", int(self.betamount/2), self.current.id , interaction.guild.id)
        else :    
            self.amounts[self.current][1] += self.betamount
            await client.db.execute("UPDATE users SET bank = bank - $1 WHERE id = $2 AND guild_id = $3", int(self.betamount), self.current.id , interaction.guild.id)
    ...
    async def bet(self , interaction , button):
        ...
        if ( self.betamount ) >  self.amounts[self.current][2]:
            self.embed.set_footer(text= f"🛑 {self.current.name} No More Money In bank , Last option Is Pack")
            await interaction.response.edit_message(embed = self.embed)
            self.embed.set_footer(text=None)
            return  
...
class patti(commands.Cog):
    ...
    async def blind_fun(self , channel , author , amount ):
        ...
        if len(self.players.get(channel , [ ])) == 0:
            ...
            await client.db.execute("UPDATE users SET bank = bank - $1 WHERE id = $2 AND guild_id = $3", amount, author.id, channel.guild.id)
    ...
    async def blind(self , ctx , amount : typing.Literal[100 , 1000 , 10000] = 1000): 
