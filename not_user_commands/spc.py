...
class SPC(commands.Cog):
    ...
    async def spc(self , ctx ):
        if len(self.players) == 0:
            msg = await ctx.reply(embed = discord.Embed(description=f"<a:811261898360094730:1063182356913078362> waiting for other player (exp <t:{int(datetime.now().timestamp()+30)}:R>)"))
